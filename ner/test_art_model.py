...
import numpy as np
sys.path.append('..')
from ner.models import bi_art_lstm_model
...
test_input = [tx, twx]
py = model.predict([twx, tx], batch_size=32, verbose=1)
target_task = __import__(target_domain + '_preprocess')
acc, f1 = target_task.evaluate(py, ty, tm)
result = source_domain + ' to ' + target_domain + ' ' + str(labeling_rates) + ' ' + str(f1)
print(result)